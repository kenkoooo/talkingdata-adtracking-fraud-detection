...
train = pd.read_csv(path+"train.csv", skiprows=range(1,129903891), nrows=55000000, usecols=train_columns, dtype=dtypes)
...
if (is_valid == True):
    ...
    model = xgb.train(params, xgb.DMatrix(x1, y1), 200, watchlist, maximize=True, early_stopping_rounds = 25, verbose_eval=5)
    del dvalid
else:
